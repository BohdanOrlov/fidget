use crate::{
    ...
    grad_slice::GradSliceAssembler, jit_shell_grad_ptr, mmap::Mmap, reg,
};
use dynasmrt::{DynasmApi, DynasmError, DynasmLabelApi, dynasm};
use fidget_core::{shell::ShellTopology, types::Grad};
...
impl GradSliceAssembler {
    ...
    fn call_fn_shell_distance(
        ...
        dynasm!(self.0.ops
            ...
            // Back up scratch values and SIMD tape registers.
            ; stp q0, q1, [sp, 0x1d0]
            ; str q2, [sp, 0x1f0]
            ; stp q8, q9, [sp, 0x50]
            ; stp q10, q11, [sp, 0x70]
            ...
            // Store Grad arguments into spill slots above the fixed frame.
            ; str Q(reg(x_reg)), [sp, 0x220]
            ; str Q(reg(y_reg)), [sp, 0x230]
            ; str Q(reg(z_reg)), [sp, 0x240]

            // Load helper address into x5 and shell pointer into x0.
            ...
            // Copy helper output into the out register before restoring q0-q2.
            ; ldr Q(reg(out_reg)), [sp, 0x220]

            // Restore scratch values and argument registers.
            ; ldp q0, q1, [sp, 0x1d0]
            ; ldr q2, [sp, 0x1f0]
            ; mov x0, x20
            ; mov x1, x21

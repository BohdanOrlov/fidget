    ...
    <E as TracingEvaluator>::Data: Transformable,
{
    ...
    /// Backwards-compatible alias for [`eval_with_vars`](Self::eval_with_vars).
    #[inline]
    pub fn eval_v<F: Into<E::Data> + Copy, V: Into<E::Data> + Copy>(
        ...
        self.eval_with_vars(tape, x, y, z, vars)
    }
...
/// Converts older unbound shape inputs into a [`BoundShape`].
///
/// This preserves the pre-bind API for shapes that use only `X`, `Y`, and `Z`,
/// while still allowing callers to pass an explicitly bound shape when extra
/// variables are present.
pub trait IntoBoundShape<'a> {
    /// Evaluator function type carried by this shape.
    type Function: Function;

    /// Converts this value into a bound shape.
    fn into_bound_shape(
        self,
    ) -> Result<BoundShape<'a, Self::Function, f32>, MissingVar>;
}

impl<'a, F: Function> IntoBoundShape<'a> for Shape<F> {
    type Function = F;

    #[inline]
    fn into_bound_shape(self) -> Result<BoundShape<'a, F, f32>, MissingVar> {
        self.try_into()
    }
}

impl<'a, F: Function + Clone> IntoBoundShape<'a> for &'a Shape<F> {
    type Function = F;
...
impl<'a, F: Function> IntoBoundShape<'a> for BoundShape<'a, F, f32> {
    type Function = F;
...
impl<'a, F: Function + Clone> IntoBoundShape<'a>
    for &'a BoundShape<'a, F, f32>
{
    type Function = F;
